...
std::pair<std::vector<BasisState>, SvmResult> run_deuteron_svm(const std::vector<bool>& relativistic, ld b_range, ld b_form, ld S, const std::vector<ld>& box_strengths) {
    ...
    for (ld ho_k : box_strengths) {
        ...
        for(int i=0; i<2; i++) competitive_search(local_basis, channel_templates, 10000, b_range, b_form, S, relativistic, ho_k, n_vals);
        
        // 2. Fast sweep on just these states!
        sweep_optimize_basis(local_basis, b_form, b_range, S, relativistic, convergence_energies, 20, 1e-4, ho_k, n_vals);
    ...
    sweep_optimize_basis(grand_basis, b_form, b_range, S, relativistic, convergence_energies, 20, 1e-4, 0.0, n_vals);
