#!/usr/bin/env python3
"""
sweep_b_form.py - Orchestrates b_form parameter sweeps with integrated plotting

Runs multiple deu instances with varying b_form, collects results, plots automatically.

Usage:
  python3 scripts/sweep_b_form.py \
    --S 39.17 --b_range 2.55 \
    --b_form_min 1.0 --b_form_max 1.6 --b_form_steps 8 \
    [--jobs 4]
"""
...
class BFormSweep:
    """Manages b_form parameter sweep with integrated plotting"""
    
    def __init__(self, fixed_params, sweep_params, num_jobs=1):
        self.scan_type = "b_form"
        self.fixed_params = fixed_params
        self.sweep_params = sweep_params
        self.num_jobs = num_jobs
        self.results_dir = f"results/energy_sweep_b_form"
        self.run_dir = self.results_dir
        
    def generate_parameter_combinations(self):
        """Generate all b_form parameter combinations"""
        combinations = []
        
        b_form_values = np.linspace(
    ...
    def aggregate_results(self, run_results):
        ...
        # Sort by b_form for readability
    ...
    def print_summary(self, run_results):
        ...
        print("\n" + "="*80)
        print(f"SWEEP SUMMARY: b_form parameter sweep")
        print("="*80)
    ...
    def plot_energy_vs_b_form(self, csv_file):
        ...
        b_form_vals, energies, radii = zip(*sorted_data)
        
        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(10, 8))
        
        ax1.plot(b_form_vals, energies, 'o-', linewidth=2, markersize=8, label='Computed')
        ax1.axhline(y=-2.224, color='r', linestyle='--', linewidth=2, label='Target (-2.224 MeV)')
        ax1.set_xlabel('Form Factor Parameter $b_{form}$ (fm)', fontsize=12)
        ax1.set_ylabel('Energy (MeV)', fontsize=12)
        ax1.set_title('Deuteron Ground State Energy vs Form Factor', fontsize=14, fontweight='bold')
        ax1.grid(True, alpha=0.3)
        ax1.legend(fontsize=10)
        
        if radii and any(r > 0 for r in radii):
            ax2.plot(b_form_vals, radii, 's-', linewidth=2, markersize=8, color='green', label='Computed')
            ax2.axhline(y=2.128, color='r', linestyle='--', linewidth=2, label='Target (2.128 fm)')
            ax2.set_xlabel('Form Factor Parameter $b_{form}$ (fm)', fontsize=12)
            ax2.set_ylabel('Charge Radius (fm)', fontsize=12)
            ax2.set_title('Charge Radius vs Form Factor', fontsize=14, fontweight='bold')
    ...
    def run(self):
        ...
        print(f"Starting b_form parameter sweep")
        print(f"  Fixed S: {self.fixed_params['S']}")
        print(f"  Fixed b_range: {self.fixed_params['b_range']}")
        print(f"  b_form range: [{self.sweep_params['b_form_min']}, {self.sweep_params['b_form_max']}]")
        ...
        if agg_path:
            ...
            self.plot_energy_vs_b_form(agg_path)
...
def main():
    parser = argparse.ArgumentParser(
        description="b_form parameter sweep orchestrator for deuteron SVM solver"
    )
    ...
    parser.add_argument("--b_range", type=float, required=True,
                        help="Fixed b_range (box range parameter)")
    ...
    sweep = BFormSweep(fixed_params, sweep_params, args.jobs)
    agg_path = sweep.run()
