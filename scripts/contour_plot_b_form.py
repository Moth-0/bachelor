...
Usage:
  python3 scripts/contour_plot_b_form.py \
    --b_form_min 1.0 --b_form_max 1.5 --b_form_steps 10 \
...
def main():
    parser = argparse.ArgumentParser(description="Generate Dynamic Slanted 2D Contour Plot for b_form vs S")
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional comma-separated list of per-slice S centers (length must equal b_form_steps). If provided, disables dynamic tracking.",
    )
    ...
    parser.add_argument("--b_range", type=float, default=2.5)
    ...
    if args.S_anchors is not None:
        try:
            ...
            return 2
        if len(s_anchors) != args.b_form_steps:
            print(
                f"ERROR: --S_anchors length ({len(s_anchors)}) must equal --b_form_steps ({args.b_form_steps})",
                file=sys.stderr,
            )
            return 2
    ...
    if args.refine_iters > 0 and pending_refine:
        ...
        for it in range(args.refine_iters):
            ...
            for entry in active:
                ...
                entry["s_next"] = s_next
                proposals.append((entry["b"], s_next, args.b_range, None, results_dir))

            # Assign unique run ids.
            batch_args = []
            for (b, s_next, b_range, _rid, resdir) in proposals:
                batch_args.append((b, s_next, b_range, run_id, resdir))
                run_id += 1
